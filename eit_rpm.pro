...
pro timebinning_cycle
 ...
 timebinning,'','/data1/tomography/DATA/euvi/CR2082/B171/','listB171',4,/euvi
 timebinning,'','/data1/tomography/DATA/euvi/CR2082/B195/','listB195',4,/euvi
 timebinning,'','/data1/tomography/DATA/euvi/CR2082/B284/','listB284',4,/euvi
...
;-------------------------------------------------------------------------------------
pro eit_process_solmin

eit_process,directory='/data1/tomography/DATA/eit/CR1919/171/',listfile='list.171.raw',nfiles=108,/rebin,binfactor=2,/normalize,/despike,/norot
eit_process,directory='/data1/tomography/DATA/eit/CR1919/195/',listfile='list.195.raw',nfiles=603,/rebin,binfactor=2,/normalize,/despike,/norot
eit_process,directory='/data1/tomography/DATA/eit/CR1919/284/',listfile='list.284.raw',nfiles=823,/rebin,binfactor=2,/normalize,/despike,/norot
eit_process,directory='/data1/tomography/DATA/eit/CR1919/304/',listfile='list.304.raw',nfiles=669,/rebin,binfactor=2,/normalize,/despike,/norot

eit_process,directory='/data1/tomography/DATA/eit/CR1915/171/',listfile='list.171.raw',nfiles=139,/rebin,binfactor=2,/normalize,/despike,/norot
eit_process,directory='/data1/tomography/DATA/eit/CR1915/195/',listfile='list.195.raw',nfiles=504,/rebin,binfactor=2,/normalize,/despike,/norot
eit_process,directory='/data1/tomography/DATA/eit/CR1915/284/',listfile='list.284.raw',nfiles=126,/rebin,binfactor=2,/normalize,/despike,/norot
eit_process,directory='/data1/tomography/DATA/eit/CR1915/304/',listfile='list.304.raw',nfiles=224,/rebin,binfactor=2,/normalize,/despike,/norot

eit_process,directory='/data1/tomography/DATA/eit/CR1914/171/',listfile='list.171.raw',nfiles=196,/rebin,binfactor=2,/normalize,/despike,/norot
eit_process,directory='/data1/tomography/DATA/eit/CR1914/195/',listfile='list.195.raw',nfiles=391,/rebin,binfactor=2,/normalize,/despike,/norot
eit_process,directory='/data1/tomography/DATA/eit/CR1914/284/',listfile='list.284.raw',nfiles=192,/rebin,binfactor=2,/normalize,/despike,/norot
eit_process,directory='/data1/tomography/DATA/eit/CR1914/304/',listfile='list.304.raw',nfiles=292,/rebin,binfactor=2,/normalize,/despike,/norot

eit_process,directory='/data1/tomography/DATA/eit/CR1911/171/',listfile='list.171.raw',nfiles=134,/rebin,binfactor=2,/normalize,/despike,/norot
eit_process,directory='/data1/tomography/DATA/eit/CR1911/195/',listfile='list.195.raw',nfiles=197,/rebin,binfactor=2,/normalize,/despike,/norot
eit_process,directory='/data1/tomography/DATA/eit/CR1911/284/',listfile='list.284.raw',nfiles= 88,/rebin,binfactor=2,/normalize,/despike,/norot
eit_process,directory='/data1/tomography/DATA/eit/CR1911/304/',listfile='list.304.raw',nfiles=113,/rebin,binfactor=2,/normalize,/despike,/norot


return
end

pro eit_process,DIRECTORY=DIRECTORY,LISTFILE=LISTFILE,NFILES=NFILES,$
                REBIN=REBIN,BINFACTOR=BINFACTOR,DESPIKE=DESPIKE,ROT=ROT,NOROT=NOROT,NORMALIZE=NORMALIZE;,$
;               DECON=DECON,PHOTONS=PHOTONS,DNSEC=DNSEC
...
read_eit, directory+filenames(i), index, data
...
;-----------------------------------------
;if NOT keyword_set(despike) then eit_prep, directory+filenames(i) , hdr , image 
;if     keyword_set(despike) then eit_prep, directory+filenames(i) , hdr , image , /cosmic 

 if NOT keyword_set(despike) then eit_prep, index, hdr, image, data = data 
 if     keyword_set(despike) then eit_prep, index, hdr, image, data = data , /cosmic
;-----------------------------------------
...
if keyword_set(norot) then rotstring='norot'
...
if keyword_set(rebin) AND index.NAXIS1 eq 1024 AND binfactor gt 1 then begin
...
if keyword_set(despike) then newfilename=newfilename+'.DESPIKE'
...
if     keyword_set(normalize) then begin
 divide_by_ck0,image,hdr,newimage
 image=newimage
...
tvscl,alog10(monitor_image)
skipgraph2:

filename=directory+newfilename
MWRITEFITS, hdr,image, outfile=filename 

goto,skipgraph3
record_gif,directory,newfilename+'.gif'
skipgraph3:
...
pro divide_by_ck0,image,hdr,newimage
; Clear Filter is assumed
...
if flag eq 0 then begin
;-----------------------------------------
;eit_prep, index, hdr1, image1, data = data;, /no_roll
 if dspk eq 0 then eit_prep, index, hdr , image , data = data
 if dspk eq 1 then eit_prep, index, hdr , image , data = data, /cosmic 
;/response_norm:  apply flat-field
;/filter_norm:    propertly treat different transmittivity of the various filters 
;/normalize:      ?
;-----------------------------------------
...
; REMOVE /NORMAL_OFF if scale up to OPEN is desired here (only approximately, see my notes)
 if dspk eq 0 then begin
   secchi_prep,data_dir+filenames(i),hdr,image,/DN2P_OFF,/CALIMG_OFF,/NORMAL_OFF
 endif
...
lastfile:
  ...
  filename=data_dir+strmid(string(file(ilabel)),0,8)+'.'+timestrings(nib/2)+'.'+$
           strmid(file(i-1),20,strlen(file(i-1))-24)+'.b'+strmid(string(ntb),7,1)+'.fts'
