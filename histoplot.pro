...
;Histograma convencional, se le da vectores de entrada en data1 y/o data2
  ...
  if not keyword_set(min)   then min = min(data1)
  ...
  f1 = histogram(data1,min=min,max=max,nbins=nbins,locations=vbin1) & float(f1) = float(f1) / float(total(f1))
  if keyword_set(data2) then begin
     ...
     f2 = histogram(data1,min=min,max=max,nbins=nbins,locations=vbin2) & float(f2) = float(f2) / float(total(f2))
  endif
  azul = 100
  rojo = 200
  ...
  if not keyword_set(data2) then begin
     xyouts,0.8*[1,1,1,1],1-[0.18,0.25,0.32,0.38],['m='+strmid(string(med1),4,6),'!9m!3='+strmid(string(avg1),4,6),'!9s!3/!9m!3='+strmid(string(stdev1),4,6),'N='+strmid(string(cant1),7,7)],/normal,charthick=1,Font=0,charsize=2.2
     xyouts,0.8*[1],1.-[0.1],[label1],/normal,color=[azul],charthick=3
  endif
  if keyword_set(data2) then begin
     xyouts,0.8*[1,1,1,1],1-[0.18,0.25,0.32,0.38],['m='+strmid(string(med1),4,6)+'&'+strmid(string(med2),4,6),'!9m!3='+strmid(string(avg1),4,6)+'&'+strmid(string(avg2),4,6),'!9s!3/!9m!3='+strmid(string(stdev1),4,6)+'&'+strmid(string(stdev2),4,6),'N='+strmid(string(cant1),7,7)+'&'+strmid(string(cant2),7,7)],/normal,charthick=1,Font=0,charsize=2.2
